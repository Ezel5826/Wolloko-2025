...
object colectivo {
    ...
    method avanzar(){  
        ...
        if (self.recorridoTerminado())return false
        if (self.obtenerGente()!=0){self.subir_gente(self.obtenerGente())}
        if (ruta.parada_actual() + avance >= 0 && ruta.parada_actual() + avance <= ruta.paradas().size()-1){ruta.parada_actual(ruta.parada_actual() + avance)}
    ...
    method bajar_pasajeros() {//anda
      if (self.recorridoTerminado() || (ruta.parada_actual() == 0 && vueltas != 0) || self.llegaTerminal() ){
        ...
        pasajeros_subidos = 0
        if (!self.recorridoTerminado() || (ruta.parada_actual() == 0 && vueltas != 0)){vueltas += 1 ruta.parada_actual(0)}
        self.recargar_nafta(ruta.recargarNafta())
      }
    }   

    method subir_gente(cantidad_pasajeros) {//anda
        if (((self.gasto_nafta_paradas_restantes(pasajeros_subidos+cantidad_pasajeros,ruta.paradas(),ruta.parada_actual()) <= nafta_actual ) && (pasajeros_subidos + cantidad_pasajeros <= max_pasajeros) && (ruta.parada_actual() != 0 || ruta.parada_actual()!=ruta.paradas().size()-1))){
    ...
    method gasto_nafta_paradas_restantes(subir, paradas, paradaActual) {
        ...
        if ((ruta.paradas().size()-1 - iterador  == (ruta.paradas().size()-1 - paradaActual)-1) && paradaActual >= 0)//ruta.paradas_restantes()-1
        {
    ...
    method recorridoTerminado() = ruta.parada_actual() == ruta.paradas().size()-1 && cant_gente_sobrante==0

    method llegaTerminal() = ruta.parada_actual() == ruta.paradas().size()-1
